...
export default function NoticeCard({ notice }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`admin-card p-5 relative ${notice.is_pinned ? 'ring-2 ring-primary' : ''}`}
    >
      {notice.is_pinned && (
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        ...
        <span className="inline-flex items-center gap-1 px-2 py-1 bg-admin-surface-2 text-admin-muted text-xs font-medium rounded-full">
      ...
      <p className="mt-3 text-admin-muted text-sm line-clamp-3">{notice.description}</p>
