    ...
    <div>
            ...
            <tbody className="divide-y divide-slate-100">
              {members.map((m) => (
                <tr key={m.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{m.position}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{m.team}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{m.department}</td>
                  <td className="px-6 py-4 flex gap-2">
                    <button onClick={() => openEdit(m)} className="p-2 hover:bg-slate-100 rounded-lg"><Edit2 size={16} className="text-primary" /></button>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Member' : 'Add Member'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-slate-700 mb-1">Team</label><input {...register('team')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" placeholder="e.g. Executive, Technical, Media" /></div>
          <div><label className="block text-sm font-medium text-slate-700 mb-1">Department</label><select {...register('department')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl"><option value="">Select</option>{DEPARTMENTS.map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
