            ...
            <tbody className="divide-y divide-slate-100">
              {queries.map((q) => (
                <tr key={q.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{q.name}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{q.category}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      q.status === 'Resolved' ? 'bg-green-100 text-green-700' :
                      q.status === 'Closed' ? 'bg-slate-100 text-slate-600' :
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title="Query Details" size="lg">
        {selected && (
          <div className="space-y-4">
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Reply</label><textarea value={reply} onChange={(e) => setReply(e.target.value)} rows={3} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" placeholder="Type your reply..." /></div>
