              ...
              {certs.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 flex gap-2">
                    <button onClick={() => openEdit(c)} className="p-2 hover:bg-slate-100 rounded-lg"><Edit2 size={16} className="text-primary" /></button>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Certificate' : 'Add Certificate'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Category</label><input {...register('category')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" placeholder="e.g. Certificate, Award, Rating" /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Year</label><input {...register('year')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" placeholder="e.g. 2024" /></div>
          </div>
          <div><label className="block text-sm font-medium text-slate-700 mb-1">Description</label><textarea {...register('description')} rows={2} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" /></div>
          <div><label className="block text-sm font-medium text-slate-700 mb-1">Thumbnail URL</label><input {...register('thumbnail_url')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" /></div>
          <div><label className="block text-sm font-medium text-slate-700 mb-1">Document URL</label><input {...register('document_url')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" /></div>
