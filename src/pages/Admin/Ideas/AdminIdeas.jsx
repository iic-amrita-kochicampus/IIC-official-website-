            ...
            <tbody className="divide-y divide-slate-100">
              {ideas.map((i) => (
                <tr key={i.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{i.name}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{i.department}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{formatDate(i.created_at)}</td>
                  <td className="px-6 py-4">
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title="Review Idea" size="lg">
        {selected && (
          <div className="space-y-4">
            ...
            <div><span className="font-semibold">Problem Statement:</span><p className="text-slate-600 mt-1">{selected.problem_statement}</p></div>
            <div><span className="font-semibold">Proposed Solution:</span><p className="text-slate-600 mt-1">{selected.proposed_solution}</p></div>
            <div><span className="font-semibold">Expected Impact:</span><p className="text-slate-600 mt-1">{selected.expected_impact}</p></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Admin Remarks</label><textarea value={remark} onChange={(e) => setRemark(e.target.value)} rows={3} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" /></div>
