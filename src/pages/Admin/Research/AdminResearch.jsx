        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-x-auto">
          <table className="w-full text-left min-w-[600px]">
            <thead className="bg-slate-50"><tr>
              ...
              <th className="px-6 py-4 text-sm font-semibold text-slate-600">Researcher</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {research.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{r.researcher}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${r.status === 'Ongoing' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{r.status}</span>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">{r.category}</td>
                  <td className="px-6 py-4 flex gap-2">
                    <button onClick={() => openEdit(r)} className="p-2 hover:bg-slate-100 rounded-lg"><Edit2 size={16} className="text-primary" /></button>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Research' : 'Add Research'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Researcher</label><input {...register('researcher')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" /></div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Category</label><input {...register('category')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl" /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Status</label><select {...register('status')} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl"><option>Ongoing</option><option>Completed</option><option>Published</option></select></div>
          </div>
